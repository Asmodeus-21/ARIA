        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-8">
          {pricingPlans.map((plan) => (
            ...
            >
              ...
              <div className="mb-6">
                <h3 className={`text-xl font-bold transition-colors duration-300 ${plan.isFeatured ? 'text-blue-600' : 'text-gray-900 group-hover:text-blue-600'}`}>{plan.name}</h3>
                <p className="mt-2 text-sm text-gray-500 leading-snug min-h-[40px]">{plan.description}</p>
              </div>
              
              <div className="mb-8">
                <div className="flex items-baseline">
                    <span className="text-4xl font-extrabold text-gray-900 tracking-tight group-hover:scale-110 transition-transform duration-300 origin-left">{plan.price}</span>
                    {plan.price.startsWith('$') && <span className="text-sm font-medium text-gray-500 ml-1">/mo</span>}
                </div>
              </div>
              ...
              <ul className="space-y-4 mb-8 flex-1">
                {plan.features.map((feature) => (
                  <li key={feature} className="flex items-start">
                    <div className={`flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center mt-0.5 transition-colors duration-300 ${plan.isFeatured ? 'bg-blue-100 text-blue-600' : 'bg-gray-100 text-gray-400 group-hover:bg-blue-100 group-hover:text-blue-600'}`}>
